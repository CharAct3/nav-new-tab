import React from 'react';
import { Button, Form, Modal } from 'react-bootstrap';

const Upload: React.FunctionComponent = () => {
  const [file, setFile] = React.useState<File | null>(null);
  const [show, setShow] = React.useState<boolean>(false);

  const handleClose = (): void => setShow(false);
  const handleShow = (): void => setShow(true);

  const onSubmit = (): void => {
    if (file !== null) {
      const reader = new FileReader();
      reader.onload = (event: ProgressEvent<FileReader>): void => {
        if (event.target && event.target.result) {
          const groups = JSON.parse(event.target.result as string);
          chrome.storage.local.set({ groups: groups }, () =>
            console.log('Loaded')
          );
        }
      };
      reader.readAsText(file);
    }
    handleClose();
  };
  const onFileSelected: React.ChangeEventHandler<HTMLInputElement> = e => {
    if (e.target.files) {
      setFile(e.target.files[0]);
    }
  };

  return (
    <>
      <Button variant="link" className="ingress-button" onClick={handleShow}>
        <svg
          height="20"
          aria-hidden="true"
          focusable="false"
          data-prefix="fas"
          data-icon="file-upload"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 384 512"
          className="svg-inline--fa fa-file-upload fa-w-12 fa-9x"
        >
          <path
            fill="currentColor"
            d="M224 136V0H24C10.7 0 0 10.7 0 24v464c0 13.3 10.7 24 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zm65.18 216.01H224v80c0 8.84-7.16 16-16 16h-32c-8.84 0-16-7.16-16-16v-80H94.82c-14.28 0-21.41-17.29-11.27-27.36l96.42-95.7c6.65-6.61 17.39-6.61 24.04 0l96.42 95.7c10.15 10.07 3.03 27.36-11.25 27.36zM377 105L279.1 7c-4.5-4.5-10.6-7-17-7H256v128h128v-6.1c0-6.3-2.5-12.4-7-16.9z"
            className=""
          ></path>
        </svg>
      </Button>
      <Modal show={show} onHide={handleClose} centered>
        <Modal.Header closeButton>
          <Modal.Title>Upload your config file</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form className="text-center">
            <input
              name="config"
              type="file"
              id="upload"
              onChange={onFileSelected}
            />
            <label htmlFor="upload">{file ? file.name : 'Choose a file'}</label>
          </Form>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="outline-primary" onClick={onSubmit}>
            Upload
          </Button>
        </Modal.Footer>
      </Modal>
    </>
  );
};
export default Upload;
